...
/*
 * This class represents a TCP connection and manages most errors.
 * The main listening process runs in its own thread.
 * The class has 4 public events in delegates that are executed into independent threads to allow the main listening thread to keep running,
 * the last event is private and it's used only to remove this connection from the server.
 * 
 * Events:
...
/* Events templates (Copy those into your code, then assign to the events of this object):
    ...
    void OnTCPOpen(TCPConnection connection)
    {}
...
public class TCPConnection
{
    ...
    // Events:
    public delegate void EventMessage(byte[] data, TCPConnection connection);                   // Event delegate with a byte array as arguments.
    public delegate void EventException(int code, string message, TCPConnection connection);    // Event delegate with int and string as arguments.
    public delegate void EventVoid(TCPConnection connection);                                   // Event delegate for generic events.
    ...
    ///<summary>Constructor</summary>
    public TCPConnection(string localIP = "", EventVoid evOpen = null, EventMessage evMessage = null, EventException evError = null, EventVoid evClose = null, byte[] eof = null)
    {
        Setup(localIP, evOpen, evMessage, evError, evClose, eof);
    }
    ///<summary>(Reserver for WSServer use) Constructor</summary>
    public TCPConnection(Socket client, EventVoid deleteMeCallback, float keepAliveTimeout)
    {
        _client = client;
        ...
        lock (_clientLock)
        {
            ...
            catch (System.Exception e)
            {
                ThreadPool.QueueUserWorkItem((object s) => {
                    onError?.Invoke(e.HResult, "[TCPConnection.Setup] " + e.Message, this);
                });
            }
        }
    }
    ...
    void Connect(IPEndPoint remoteIPEndpoint, float timeout = 5f, float keepAliveTimeout = 15f, bool disableWatchdog = false)
    {
        lock (_clientLock)
        {
            ...
            {
                WriteLine("[TCPConnection] You should call Setup() before attempt to connect.", true);
            }
        }
    }
    ///<summary>Set the connection (to send and receive)</summary>
    public void Connect(int port, string remoteIP, float timeout = 5f, float keepAliveTimeout = 15f, bool disableWatchdog = false)
    {
        try
        {
            ...
            if (IsValidPort(port))
                ...
                WriteLine("[TCPConnection.Connect] You should provide a valid port (1 to 65535).", true);
        }
        catch (System.Exception e)
        {
            ThreadPool.QueueUserWorkItem((object s) => {
                onError?.Invoke(e.HResult, "[TCPConnection.Connect] " + e.Message, this);
            });
        }
    }
        ...
        if (_client != null && !_disconnecting)                     // Bugfix for weird "null object" when retrying connection.
        {
            ...
            catch (SocketException e)
            {
                ...
                if (e.ErrorCode != 10065 && e.ErrorCode != 10061 && e.ErrorCode != 10055 && e.ErrorCode != 10038 && e.ErrorCode != 10004)
                    ThreadPool.QueueUserWorkItem((object s) => {
                        onError?.Invoke(e.ErrorCode, "[TCPConnection.Connecting] " + e.Message, this);
                    });
            }
        ...
        while (IsConnected())
        {
            ...
            catch (SocketException e)
            {
                ...
                else if (!_disconnecting)
                {
                    ...
                    ThreadPool.QueueUserWorkItem((object s) => {
                        onError?.Invoke(e.ErrorCode, "[TCPConnection.ReceiveData] " + e.Message, this);
                        SafeOnClose();
                    });
                }
            }
    ...
    public void SendData(byte[] data)
    {
        if (IsConnected())
        {
            ...
            {
                ThreadPool.QueueUserWorkItem((object s) => {
                    onError?.Invoke(e.HResult, "[TCPConnection.SendData] " + e.Message, this);
                });
            }
        }
    }
    ...
    void SafeOnOpen()
    {
        ...
        {
            ThreadPool.QueueUserWorkItem((object s) => {
                onError?.Invoke(e.HResult, "[TCPConnection.SafeOnOpen] " + e.Message, this);
            });
        }
    }
    ...
    void SafeOnMessage(byte[] message)
    {
        ...
        {
            ThreadPool.QueueUserWorkItem((object s) => {
                onError?.Invoke(e.HResult, "[TCPConnection.SafeOnMessage] " + e.Message + " - Received: " + (ByteArrayToString(message)), this);
            });
        }
    }
    ...
    public void SafeOnClose()
    {
        ...
        {
            ThreadPool.QueueUserWorkItem((object s) => {
                onError?.Invoke(e.HResult, "[TCPConnection.SafeOnClose] " + e.Message, this);
            });
        }
    }
    ...
    public IPEndPoint AddressParser(string ipOrUrl = "ipv4", int port = 0)
    {
        ...
        else
        {
            ...
            if (address == null)
            {
                ...
                {
                    ThreadPool.QueueUserWorkItem((object s) => {
                        onError?.Invoke(e.HResult, "[TCPConnection.AddressParser] " + e.Message, this);
                    });
                }
            }
            else
