...
/*
 * This class represents a UDP connection and manages most errors and applies known solutions for some rare issues.
 * The main listening process runs in its own thread.
 * The class has 4 events in delegates that are executed into independent threads to allow the main listening thread keep running.
 * 
 * Events:
 * onOpen = Fired when the connection is properly established (In UDP it means "listening" instead of "connected").
 * onMessage(byte[]) = Fired when a message has arrived (if no event is assigned, the message is stored in _rxBuffer).
 * onError(int, string) = Fired when an error has occurred.
...
// The UDP connection class:
public class UDPConnection
{
    // Main objects:
    public volatile UdpClient _client;                                                          // Connection object.
    public volatile UDPConnection _auxClient;                                                   // The auxiliary port to allow both protocols (IPv4 and IPv6).
    ...
    // Events:
    public delegate void EventMessage(byte[] data, string ip, UDPConnection connection);        // Event delegate with a byte array and a string as arguments.
    public delegate void EventException(int code, string message, UDPConnection connection);    // Event delegate with int and string as arguments.
    public delegate void EventVoid(UDPConnection connection);                                   // Event delegate for generic events.
    ...
    public void Setup(int port, string localIP = "", EventVoid evOpen = null, EventMessage evMessage = null, EventException evError = null, EventVoid evClose = null)
    {
        ...
        if (IsValidPort(port))
        {
            ...
            try
            {
                switch (_localIP)
                {
                    ...
                    case "":
                        ...
                        else
                        {
                            ...
                            _auxClient = new UDPConnection(_port, "ipv6", null, OnMessageAux, OnErrorAux, OnCloseAux);
                        }
                        break;
                    default:
                ...
                // On Apple assume the maximum buffer is available, it can't be measured this way:
                _bufferSize = 65504;
    ...
    void OnMessageAux(byte[] data, string remoteIP, UDPConnection connection)
    {
    ...
    void OnErrorAux(int error, string message, UDPConnection connection)
    {
    ...
    void OnCloseAux(UDPConnection connection)
    {
        Disconnect();
        // This is to detect erros in the code assigned by the user to this event:
        try
